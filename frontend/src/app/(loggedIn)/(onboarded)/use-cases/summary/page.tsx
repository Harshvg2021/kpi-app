...
import {
  ...
  exportToPDF,
  exportToPowerPoint,
} from "@/lib/convertor/useCaseConvertor";
import { useUseCaseList } from "@/context/UseCaseProvider";

const Page = () => {
  const router = useRouter();
  const { selectedList } = useUseCaseList();
    ...
    <div className="">
      <Card className="w-full max-w-4xl min-w-[80vw] flex flex-col h-full min-h-[80vh] mx-auto p-6 bg-white rounded-lg shadow-lg">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Summary (Use Case)</h1>
        ...
        <div className="flex self-end justify-end items-center gap-4 mt-6">
          <Button
            onClick={() => router.push(`/use-cases/list?${search.toString()}`)}
            variant="outline"
